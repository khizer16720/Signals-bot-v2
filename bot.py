...
app = Flask(__name__)
REPORT = "<h3>⏳ 5 Coins ka Advanced Backtest chal raha hai...</h3>"
...
def run_backtest(df):
    ...
    for i in range(22, len(df) - 1):
        rsi = df['RSI'][i]
        # RSI Filter: Sirf 35 aur 65 ke beech
        if not (35 < rsi < 65): continue
        
        signal = None
        if df['EMA9'][i] > df['EMA21'][i] and df['EMA9'][i-1] <= df['EMA21'][i-1]: signal = "LONG"
        elif df['EMA9'][i] < df['EMA21'][i] and df['EMA9'][i-1] >= df['EMA21'][i-1]: signal = "SHORT"
        
        if signal:
            entry = df['close'][i]
            sl = entry - (df['ATR'][i] * 2) if signal == "LONG" else entry + (df['ATR'][i] * 2)
            tp = entry + (df['ATR'][i] * 4) if signal == "LONG" else entry - (df['ATR'][i] * 4)
...
def generate_report():
    ...
    REPORT = f"""
    <h1>📊 Advanced EMA + RSI Scalping Report</h1>
    <table border="1">
